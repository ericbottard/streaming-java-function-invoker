...
public class GrpcServerAdapter extends ReactorRiffGrpc.RiffImplBase {
    ...
    public Flux<OutputSignal> invoke(Flux<InputSignal> request) {
        return request
                .switchOnFirst((first, stream) -> {
                    ...
                    List<List<MediaType>> accept = getExpectedOutputContentTypes(first);
                    MediaType[] downgraded = accept.stream().map(list -> list.get(0)).toArray(MediaType[]::new);
    ...
    private List<List<MediaType>> getExpectedOutputContentTypes(Signal<? extends InputSignal> first) {
        ...
        return expectedContentTypesList.stream().map(MediaType::parseMediaTypes).collect(Collectors.toList());
    }
